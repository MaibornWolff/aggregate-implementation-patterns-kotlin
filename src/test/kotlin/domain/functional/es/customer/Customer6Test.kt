...
import domain.functional.es.customer.Customer6.changeEmailAddress
import domain.functional.es.customer.Customer6.confirmEmailAddress
...
internal class Customer6Test {
    ...
    private fun WHEN_ConfirmEmailAddress_With(confirmationHash: Hash?) {
        ...
        try {
            recordedEvents = eventStream?.let { confirmEmailAddress(it, command) }
    ...
    private fun WHEN_ChangeEmailAddress_With(emailAddress: EmailAddress?) {
        ...
        try {
            recordedEvents = eventStream?.let { changeEmailAddress(it, command) }
